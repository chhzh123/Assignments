...
def create_image_and_label(nx,ny, cnt = 10, r_min = 5, r_max = 50, border = 40, sigma = 20, rectangles=False):
    
    
    image  = np.ones((nx, ny, 1))
    label  = np.zeros((nx, ny, 3), dtype=np.bool)
    mask   = np.zeros((nx, ny), dtype=np.bool)
    border = int((nx + ny)/2 * 0.2)
    r_min  = int((nx + ny)/2 * 0.05)
    #print(r_min)
    r_max  = int((nx + ny)/2 * 0.3)
    #print(r_max)
    for _ in range(cnt):
