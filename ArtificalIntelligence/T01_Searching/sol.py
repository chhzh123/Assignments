...
while not q.empty():
	...
	for pair in [(0,1),(0,2),(0,3),(1,0),(1,1),(2,0),(2,1),(3,0)]: # boat ensure
		if (b == 1 and (pair[0] > m or pair[1] > c)) or (b == 0 and (pair[0] > 5 - m or pair[1] > 5 - c)):
			continue
		if b == 0:
			newM = m + pair[0]
			newC = c + pair[1]
			...
			newC = c - pair[1]
			newB = 0
		if (newM != 0 and newM < newC) or (5 - newM != 0 and 5 - newM < 5 - newC) or newM < 0 or newC < 0 or newM > M or newC > M:
			continue
		...
		if (newM,newC,newB) not in cycle:
			...
			print(newM,newC,newB,"({})".format(priority))
			cycle.append((newM,newC,newB))
			q.put(((priority,newM+newC-2*newB),cnt,newM,newC,newB,step+1,(m,c,b)))
		else:
			# for i in range(step+1):
			# 	print("\t",end="")
