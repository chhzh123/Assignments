...
def get_word_score(word, n):
    ...
    s = word.lower()
    for c in s:
        res += SCRABBLE_LETTER_VALUES[c]
    res *= max(7*len(word)-3*(n-len(word)),1)
    return res
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while not len(new_hand) == 0:
        # Display the hand
        print("Current Hand: ",end='')
        display_hand(new_hand)
        
        # Ask user for input
        s = input("Please enter a word, or \"!!\" to indicate that you are done: ")
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            if is_valid_word(s,new_hand,word_list):
                # Tell the user how many points the word earned,
                # and the updated total score
                score = get_word_score(s,len(new_hand))
                total_score += score
                print("\"{}\" earned {} points. Total: {} points\n".format(s,score,total_score))
            ...
            new_hand = update_hand(new_hand,s)
    ...
    while i < n_hands:
        ...
        if (i != 1) and (not flag_replay) and (input("Would you like to replay the hand? ") == "yes"):
            ...
            if not flag_sub:
                ...
                if s == "yes":
                    c = input("Which letter would you like to replace: ")
                    hand = substitute_hand(hand,c)
                    flag_sub = True
                    print()
